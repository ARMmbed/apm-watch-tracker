...
static vector<string> parse_route(string route) {
    vector<string> v;
...
bool mbed_client_define_resource(string route, string v, M2MBase::Operation opr, bool observable) {
    if (client) {
    ...
    if (objectInstances.count(segments.at(0))) {
        ...
        objects.insert(pair<string, M2MObject*>(segments.at(0), obj));
        objectInstances.insert(pair<string, M2MObjectInstance*>(segments.at(0), inst));
    }
    ...
    resources.insert(pair<string, M2MResource*>(route, res));

    return true;
}

bool mbed_client_define_resource(string route, int v, M2MBase::Operation opr, bool observable) {
    stringstream ss;
    ss << v;
...
bool mbed_client_set(string route, string v) {
    if (!resources.count(route)) {
        output.printf("[ETH] [ERROR] No such route (%s)\r\n", route.c_str());
        return false;
    }
    resources[route]->set_value((uint8_t*)v.c_str(), v.length());
    return true;
}

bool mbed_client_set(string route, int v) {
    stringstream ss;
    ss << v;
    string stringified = ss.str();
...
string mbed_client_get(string route) {
    if (!resources.count(route)) {
        output.printf("[ETH] [ERROR] No such route (%s)\r\n", route.c_str());
        return string();
    }
    ...
    uint8_t* buffIn;
    uint32_t sizeIn;
    resources[route]->get_value(buffIn, sizeIn);

    string s((char*)buffIn, sizeIn);
    return s;
}

int mbed_client_get_int(string route) {
...
bool mbed_client_define_function(string route, void(*fn)(void*)) {
    ...
    resources[route]->set_execute_function(execute_callback(fp, &FunctionPointer1<void, void*>::call));
    return true;
}

bool mbed_client_define_function(string route, execute_callback fn) {
    if (!mbed_client_define_resource(route, string(), M2MBase::POST_ALLOWED, false)) {
        return false;
    }
    ...
    resources[route]->set_execute_function(fn);
    return true;
}
